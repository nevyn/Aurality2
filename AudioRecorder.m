...
static void recordingCallback (
	...
	if (inNumPackets > 0) {
		...
		if(rec->delegateWantsHighFrequency)
			[rec recorder:rec updatedHighFrequency:freq amplitude:maxVal];
		if(rec->delegateWantsFrequencies)
...
- (id) initWithURL: fileURL {
	...
	static const char *wisdomfilepath = "/tmp/wisdom";
	FILE *wisdom = fopen(wisdomfilepath, "r");
	if(wisdom)
		fftw_import_wisdom_from_file(wisdom);
	
	plan = fftw_plan_dft_r2c_1d(self.bufferSampleCount, fft_in, fft_out, FFTW_PATIENT);
	
	if(wisdom) fclose(wisdom);
	wisdom = fopen(wisdomfilepath, "w");	
...
@synthesize delegate;
-(void)setDelegate:(id<AudioRecorderDelegate, NSObject>)del_;
{
	delegate = del_;
	delegateWantsFrequencies = [delegate respondsToSelector:@selector(recorder:updatedFrequencies:)];
	delegateWantsHighFrequency = [delegate respondsToSelector:@selector(recorder:updatedHighFrequency:)];
}
